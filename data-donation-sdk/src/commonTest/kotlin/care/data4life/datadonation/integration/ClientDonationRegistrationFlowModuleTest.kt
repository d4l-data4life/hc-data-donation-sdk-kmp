...
class ClientDonationRegistrationFlowModuleTest {
    ...
    @KtorExperimentalAPI
    @Test
    fun `Given registerDonor is called with a ProgramName and a DonorKey was added somewhere else, it just runs`() =
        runWithContextBlockingTest(GlobalScope.coroutineContext) {
            ...
            val clock = ClockStub().also {
                it.whenNow =
                    { Instant.fromEpochSeconds(SessionTokenRepositoryContract.CACHE_LIFETIME_IN_SECONDS.toLong() + 30) }
            }
            ...
            val plainPayload = IsolateState {
                mutableListOf(
                    ...
                    "{\"consentMessageJSON\":\"{\\\"consentDocumentKey\\\":\\\"d4l.sample\\\",\\\"payload\\\":\\\"${
                    Base64.encodeToString(
                        encryptedPayload.access { it[0] }
                    )
                    }\\\",\\\"signatureType\\\":\\\"consentOnce\\\"}\",\"signature\":\"\"}"
                )
            }
    ...
    @KtorExperimentalAPI
    @Test
    fun `Given registerDonor is called with a ProgramName and a DonorKey needs to be created it just runs`() =
        runWithContextBlockingTest(GlobalScope.coroutineContext) {
            // Given
            val programName = "Tomato"
            val token = "Tofu"
    ...
    private class DonorKeyStorageProviderWithKey(
        ...
        override fun load(annotations: Annotations, pipe: ResultPipe<DataDonationSDK.DonorKeyRecord?, Throwable>) {
            ...
            pipe.onSuccess(
                DataDonationSDK.DonorKeyRecord(
                    recordId = "NotImportant",
                    data = ResourceLoader.loader.load("/fixture/donation/ExampleDonorIdentity.json")
                )
            )
        }
