...
internal object ConsentSignatureTypeWithoutRevokeSerializer : KSerializer<DonationContract.ConsentSignatureType> {
    override val descriptor: SerialDescriptor = PrimitiveSerialDescriptor(
        "ConsentSignatureType",
        PrimitiveKind.STRING
    )
    private val mapping = DonationContract.ConsentSignatureType.values().associateBy {
        it.value
    }.filter {
        it.key != DonationContract.ConsentSignatureType.REVOKE_ONCE.value
    }
    ...
    override fun serialize(encoder: Encoder, value: DonationContract.ConsentSignatureType) {
        if (mapping.containsValue(value)) {
            ...
            throw CoreRuntimeError.InternalFailure("Unknown ConsentSignatureType ${value.value}.")
        }
    }
